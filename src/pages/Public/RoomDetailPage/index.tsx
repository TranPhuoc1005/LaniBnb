...
export default function RoomDetailPage() {
    ...
    const { getRoomWithLocationById, fetchRoomsWithLocation } = useRoomStore();
    const { getLocationByIdFromApi } = useLocationStore();
    const { createBooking, isCreatingBooking, createBookingSuccess, error: bookingError, clearError } = useBookingStore();
    const { user, isAuthenticated } = userAuthStore();
    
    const [roomDetail, setRoomDetail] = useState<RoomWithLocation | null>(null);
    ...
    useEffect(() => {
        const fetchRoomDetail = async () => {
            ...
            try {
                ...
                let room = getRoomWithLocationById(roomId);
                console.log("Room from store:", room);
                
                if (!room) {
                    console.log("Room not in store, fetching all rooms...");
                    await fetchRoomsWithLocation();
                    room = getRoomWithLocationById(roomId);
                    console.log("Room after fetching all:", room);
                }
                
                if (!room) {
                    console.error("Room not found after all attempts");
                    setError("Không tìm thấy thông tin phòng");
                    return;
                }

                if (!room.viTri && room.maViTri) {
                    console.log("Fetching location for room...");
                    const location = await getLocationByIdFromApi(room.maViTri);
                    if (location) {
                        room = {
                            ...room,
                            viTri: {
                ...
                console.log("Final room detail:", room);
                setRoomDetail(room);
            } catch (error) {
        ...
        fetchRoomDetail();
    }, [id, getRoomWithLocationById, fetchRoomsWithLocation, getLocationByIdFromApi]);
            ...
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    <div className="text-red-600 mb-4">
                        ...
                        <p className="text-lg font-semibold">{error || "Không tìm thấy thông tin phòng"}</p>
                    </div>
                    <button
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="text-gray-600">
                                        {roomDetail.viTri?.tenViTri} • {roomDetail.viTri?.tinhThanh || roomDetail.viTri?.quocGia}
                                    </span>
                                </div>
                            </div>
                                ...
                                <div className="border-t pt-4 mb-6">
                                    <div className="space-y-2 text-sm">
                                        <div className="flex justify-between">
                                            <span>
                                                {roomDetail.giaTien.toLocaleString()}₫ x {totalDays} đêm
                                            </span>
                                            <span>
