...
export default function ListRoom() {
    const { roomsWithLocation, loading, error, fetchRoomsWithLocation } =
        useRoomStore();
    ...
    const transformRoomsToAreas = (): AreaSection[] => {
        const rooms = (roomsWithLocation || []).filter(
            (r) =>
                r.viTri?.tenViTri?.trim() &&
                r.viTri?.tinhThanh?.trim() &&
                r.viTri?.quocGia?.trim()
        );

        const enhanced: EnhancedRoom[] = rooms.map((r, idx) => ({
            id: r.id || idx + 1,
            image: r.hinhAnh || "https://via.placeholder.com/400x300?text=No+Image",
            title: r.tenPhong || `Phòng ${idx + 1}`,
            area: r.viTri?.tenViTri || "Trung tâm",
            city: r.viTri?.tinhThanh || "TP. Hồ Chí Minh",
            country: r.viTri?.quocGia || "Việt Nam",
            ...
            khach: r.khach || 1,
            phongNgu: r.phongNgu || 1,
            giuong: r.giuong || 1,
            phongTam: r.phongTam || 1,
        }));

        const grouped = enhanced.reduce((acc, room) => {
    ...
    if (error)
        return (
            <div className="py-16 text-center">
                ...
                <Button onClick={fetchRoomsWithLocation} className="mt-4">
                    Thử lại
                </Button>
            </div>
        );
